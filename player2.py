...
AI controller for paddle2 (right side).
...
def _create_learner():
    ...
    return LearningPlayer('paddle2', 'paddle2.csv', exploitation, diff_1, diff_2)
...
def record_success(strength: float = None, angle_deg: float = None, spin_deg: float = None):
    ...
    if _last_position:
        print(f"[PLAYER2] recording success for pos {_last_position} -> {strength:.3f},{angle_deg:.2f},{spin_deg:.2f}")
